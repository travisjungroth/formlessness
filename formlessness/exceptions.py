...
class ValidationIssueMap(Mapping[str, Sequence[ValidationIssue]], ValidationIssue):
    ...
    @classmethod
    def from_issues(cls, key: str, issues: Iterable[ValidationIssue]) -> ValidationIssueMap:
        top_level_issues = []
        sub_issues = {}
    ...
    def __init__(
        ...
        self.key = key
        self.top_level_issues = top_level_issues if isinstance(top_level_issues, tuple) else tuple(top_level_issues)
    ...
    def __or__(self, other: ValidationIssueMap):
        ...
        for key, issue_map in other.sub_issues.items():
            sub_issues[key] = sub_issues[key] | issue_map if key in sub_issues else issue_map
        return ValidationIssueMap(self.key, self.top_level_issues + other.top_level_issues, sub_issues)
