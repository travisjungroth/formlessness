...
from formlessness.deserializers import HasDeserializer, Deserializer, FunctionDeserializer
...
from formlessness.types import T, D
from formlessness.validators import Validator, ChoicesValidator, is_int, is_str, is_list, each_item_is_str
...
class BasicField(
    ...
    def __init__(
        ...
        self.key = key
        self.data_validators = self.default_data_validators + tuple(extra_data_validators)
        self.object_validators = self.default_object_validators + tuple(extra_object_validators)
...
class CommaListStrField(BasicField[list[str], str]):
    ...
    default_deserializer = FunctionDeserializer(lambda x: [y for y in x.split(',') if y], 'Must be a string.')
